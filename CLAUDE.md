...
- **2026-07-03** — Production bug-sweep (branch `fix/production-bug-sweep`, off `ios-testflight-prep`).
  - **Demand-based pricing REPLACES host-set pricing** (reverses the §10 "dynamic pricing" out-of-scope note). Rates come from the EDNA financial model surge tiers (time-of-day, America/Los_Angeles): off-peak 49¢, standard 68¢, peak 86¢ ($/kWh). `apps/api/src/lib/demand-pricing.ts`. The rate is LOCKED on `Booking.ratePerKwhCents` at request/modify time; `settle-session` bills the locked rate (falls back to `charger.pricePerKwhCents` for legacy rows). `estimateBooking(rateCents, powerKw, start, end)` now takes an explicit rate. `charger.get`/`nearby` return a computed `currentRateCents`. Host price inputs + tier pricing refinements removed from `packages/schemas/src/charger.ts`.
  - **Notifications are now persisted** (`Notification` model + `NotificationKind` enum). `apps/worker` writes a row per push; `apps/api/src/routers/notification.ts` exposes list/unreadCount/markRead/markAllRead. Mobile screen deep-links by kind + role (role passed as `?role=` from the profile menu).
  - **Avatar upload** via `apps/api/src/lib/supabase.ts` (service-role client → public `avatars` bucket). `auth.uploadAvatar` (base64, ≤3MB) + `auth.updateProfile`. Mobile: `expo-image-picker` + `expo-store-review` added; `Avatar` renders `uri`.
  - **Phone auth fix**: registered the `@react-native-firebase/auth` config plugin (applies the iOS openURL fix so the reCAPTCHA redirect reaches Firebase + registers the Encoded App ID URL scheme), deleted the harmful `app/firebaseauth/link.tsx` band-aid, set `aps-environment=production`. STILL NEEDS (owner/console): enable Phone provider on `ednacharge-a13d8`, upload APNs .p8 to Firebase, add test phone numbers.
  - **Keyboard fix**: `Screen` wraps scrollable forms in ONE `KeyboardAvoidingView` with an in-flow `CTABar` sibling (via `inFlow` prop) — the submit button can no longer overlap the form. Non-scroll screens keep the pinned bar.
  - **Migration** `supabase/migrations/20260703000000_demand_pricing_and_notifications.sql` — apply to prod via psql / `prisma db push` (adds `Booking.ratePerKwhCents`, `NotificationKind`, `Notification` + RLS). Not applied to prod yet.
  - Also: payment-method removal (`payment.detachPaymentMethod`), working Help/FAQ search, in-app native Terms/Privacy (`app/(shared)/legal.tsx` + `src/content/legal.ts`), removed the driver-map dead Bolt button + the tab-bar selected dot, "no chargers in this area" map indicator, fixed `support@endacharges.com` → `support@ednacharge.com` (canonical domain is **ednacharge.com**, confirmed by the financial model).
